...
"""Build extraction treatments from one existing Common Crawl DOCX fetch.

The fetched source must be the stage root containing ``.artifact.json`` and a
``data`` directory. Use its ``gs://`` path when data and compute are in the same
...
reads that same immutable input, then runs language identification and
normalization independently::
    ...
    uv run --package marin-core --extra datakit python \
      ...
      --fetched-step-path gs://bucket/run/fetched_HASH \
...
def processing_variants(
    ...
    *,
    ...
    fetched_handle = ArtifactStep.adopt(
        ...
        fetched_version,
        source=fetched_step_path.rstrip("/"),
        kind=Artifact,
    )
    ...
    for method in methods:
        ...
        extraction = StepSpec(
            ...
            fn=remote(
                partial(
                    extract_common_crawl_docx,
                    fetched_input_path=fetched_handle.path(),
                    config=config,
                    extractor=method,
                ),
